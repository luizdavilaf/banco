...
from
	...
	--WHERE nf.data > CURRENT_DATE - CAST ('3 month' AS interval)
	group by 2,3,4,5,6
...
from 
(select nota_fiscal.quantidade as qtde, age(cliente.nascimento) as idade, cliente.nascimento as nascimento,cliente.genero as genero,nota_fiscal.descricao_produto
...
-----------------------------------------------------------------------------------------------------------
			select um.maior_valor_ultimo_mes, um.funcionario, pm.maior_valor_penultimo_mes, pm.funcionario
			from
   				(select distinct sum(nf.valor-v.desconto_gerente ) as maior_valor_ultimo_mes, nf.nome_funcionario as funcionario
				from nota_fiscal nf
					 join venda v on nf.cod_venda = v.cod_venda				
				WHERE nf.data between cast('2021-07-01' as date) AND cast('2021-07-31' as date)
				group by 2
				order by 1 desc
				limit 1) as um,	
			
			-- penultimo mes
			    --select pm.maior_valor, pm.funcionario
				--from
   				(select distinct sum(nf.valor-v.desconto_gerente ) as maior_valor_penultimo_mes, nf.nome_funcionario as funcionario
				from nota_fiscal nf
					 join venda v on nf.cod_venda = v.cod_venda				
				WHERE nf.data between cast('2021-06-01' as date) AND cast('2021-06-30' as date)
				group by 2
				order by 1 desc
				limit 1) as pm
			--group by 1,2			
...
select tmp2.nome,
( select 
		 case when tmp2.valor_nota > 0 and tmp2.valor_nota <= 999.99 
		 		then '-$999.99'
			  when tmp2.valor_nota > 999.99 and tmp2.valor_nota <= 4999.99
		 		then '$1000.00-$4999.99'
			  when tmp2.valor_nota > 4999.99 and tmp2.valor_nota <= 9999.99
			    then '$5000.00-$9999.99'
			  when tmp2.valor_nota > 9999.99 and tmp2.valor_nota <= 49999.99
			    then '$10000.00-$49999.99'
			  when tmp2.valor_nota > 49999.99 and tmp2.valor_nota <= 99999.99
			    then '$50000.00-$99999.99'
			  when tmp2.valor_nota > 100000.00 then '$100000.00-'
...
select temp4.cpf as funcionario, temp4.dependentes as dependentes, temp4.salario as fixo, temp4.comissao as comissao, temp4.bruto as bruto, temp4.inss as inss, temp4.irrf as irrf, 
...
--, (length(cast(cast(abs(lucro_liquido) as integer) as text))-qtde_caracteres)+1 as quantidade
--,substring(cast(abs(lucro_liquido) as text),1,cast((length(cast(cast(abs(lucro_liquido) as integer) as text))-qtde_caracteres)+1 as integer))
...
-- resolução marcelo 
	...
	select total_notas, total_contra_cheque, ((select sum(tmp2.valor_nota) as total_notas
from
(select (tmp1.nf - tmp1.nf*tmp1.desconto) as valor_nota, tmp1.mes, tmp1.ano
from
(select sum(nota_fiscal.valor) as nf, nota_fiscal.data, venda.desconto_gerente as desconto,
  nota_fiscal.data,extract('month' from nota_fiscal.data) as mes, 
extract('year' from nota_fiscal.data) as ano
...
join venda on nota_fiscal.cod_venda = venda.cod_venda
where nota_fiscal.data between '2021-01-01' and '2021-12-31'
group by 2,3,4,5) as tmp1) as tmp2))-((select sum(temp5.contra_cheque) as total_contra_cheque--, temp5.mes, temp5.ano
				from
					(select temp4.cpf as funcionario, temp4.dependentes as dependentes, temp4.salario as fixo, temp4.comissao as comissao, temp4.bruto as bruto, temp4.inss as inss, temp4.irrf as irrf, 
					(temp4.bruto - temp4.inss - temp4.irrf)::decimal(10,2) as contra_cheque, temp4.mes, temp4.ano
				from
					(select *, 
						(case
							when temp3.salario_base <= 1903.98 then (temp3.salario_base*0)::decimal(10,2)
						...
						(select *,(temp2.bruto - temp2.inss - (temp2.dependentes*189.59))::decimal(10,2) as salario_base
							from 
								...
								from
									...
									group by 1,3,6,7 order by 1) as temp1)
						as temp2) 
					as temp3) as temp4) as temp5)) as lucro_liquido
 from
(select sum(tmp2.valor_nota) as total_notas
from
(select (tmp1.nf - tmp1.nf*tmp1.desconto) as valor_nota, tmp1.mes, tmp1.ano
from
(select sum(nota_fiscal.valor) as nf, nota_fiscal.data, venda.desconto_gerente as desconto,
...
join venda on nota_fiscal.cod_venda = venda.cod_venda
where nota_fiscal.data between '2021-01-01' and '2021-12-31'
group by 2,3,4,5) as tmp1) as tmp2) as total_notas, (select sum(temp5.contra_cheque) as total_contra_cheque--, temp5.mes, temp5.ano
				from
					(select temp4.cpf as funcionario, temp4.dependentes as dependentes, temp4.salario as fixo, temp4.comissao as comissao, temp4.bruto as bruto, temp4.inss as inss, temp4.irrf as irrf, 
					(temp4.bruto - temp4.inss - temp4.irrf)::decimal(10,2) as contra_cheque, temp4.mes, temp4.ano
				from
						...
						(select *,(temp2.bruto - temp2.inss - (temp2.dependentes*189.59))::decimal(10,2) as salario_base
							from 
								...
								from
									...
									group by 1,3,6,7 order by 1) as temp1)
						as temp2) 
					as temp3) as temp4) as temp5)as total_contra_cheque;		 
