...
tabelafinal.gerente, to_char(tabelafinal.desconto,'$999g999g999d99') as desconto
    ...
    (   select tabela2.data as data,    
        ...
        (
            ...
            from
            (
                select nota_fiscal.numeronf as NF, extract(month from nota_fiscal.data) as data, (item.quantidade*item.precoun) as totalitem, 
...
select to_char(sum(tmp1.total),'$999g999g999d99') as total_primeira_quinzena, to_char(tmp3.total ,'$999g999g999d99') as total_segunda_quinzena
	from 
		...
		group by item.precoun, item.quantidade, nota_fiscal.desconto, item.comissao) as tmp1 --primeira quinzena
		 ,
(select sum (tmp2.total) as total from
	(select
		...
		group by item.precoun, item.quantidade, nota_fiscal.desconto, item.comissao) as tmp2) as tmp3 --segunda quinzena
group by 2;
    ...
    select tabultimomes.vendedor --VENDAS MAIORES QUE 10K NO ULTIMO MES
        ...
        (
            select sum(valor_nota.total_nota*valor_nota.desconto) as valornota, valor_nota.vendedor as vendedor 
                ...
                group by 2
                having sum(valor_nota.total_nota*valor_nota.desconto)>10000
        ) as tabultimomes
    intersect
...
Colocaria em risco a integridade referencial do banco. Pois se o vendedor fosse referenciado em outra tabela como 
FK não seria possível.

4.2) É necessário ter um campo preço na tabela produto e outro na tabela item da venda em uma modelagem bem feita.
Para não misturar os dados do produto de uma venda específica. Essa ação faz com que no caso de uma atualização de preço 
no produto as vendas anteriores não sejam afetadas por essa atualização.
