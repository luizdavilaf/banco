...
select tabelafinal.data as mes, tabelafinal.gerente, to_char(tabelafinal.desconto,'$999g999g999d99') as desconto
from
(   
    select  
    (case tabela2.data
...
select distinct ultimos3meses.cliente
from
(
    --ultimo mes
    (
...
(
    ...
    (
        select sum(contracheque.comissao_mes)
            from contracheque
        where data_ref between cast('2021-09-01' as date) and cast('2021-09-30' as date)
    )
,'$999g999g999d99'
...
(
    ...
    (
        ...
        full outer join --TODAS AS VENDAS
    (
        select vendas.mes as mes, vendas.totalvendas as vendas, salario.salariobruto as salario
        from
        (
            select sum(item.quantidade*item.precoun*(1-(nota_fiscal.desconto))) as totalvendas, extract(month from nota_fiscal.data) as mes                    
                from nota_fiscal
                full outer join item on item.nota_fiscal = nota_fiscal.numeronf            
            where extract(year from nota_fiscal.data) = 2021  
            group by 2
        ) as vendas
        full outer join --TODOS OS SALARIOS
        (
            select (sum(contracheque.salario)+sum(contracheque.comissao_mes)) as salariobruto, extract(month from contracheque.data_ref) as mes
                from contracheque
            where extract(year from data_ref) = 2021
            group by 2
